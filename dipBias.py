...
DROP_THRESHOLD = 0.15  # 10% drop for Buy-the-Dip
...
DIP_YEARS = 3 # Number of years to check for dips
ACCURACY = 0.9 # Portion of times you'll be able to forecast whether a market will have a dip
...
# Function to check if a dip occurs within the first X years
def has_dip_in_first_x_years(prices, x_years, drop_threshold):
    # Calculate the end date for the first X years
    end_date = prices.index[0] + timedelta(days=365.25 * x_years)
    
    # Extract the first X years of prices
    first_x_years_prices = prices[:end_date]
    
    # Check for a dip
    last_high = first_x_years_prices.iloc[0]
    for price_value in first_x_years_prices:
        if price_value > last_high:
            last_high = price_value
        elif price_value <= last_high * (1 - drop_threshold): # Dip occurred
...
for _ in range(NUM_SIMULATIONS):
    ...
    # Check if a dip occurs in the first DIP_YEARS
    if not has_dip_in_first_x_years(window_prices, DIP_YEARS, DROP_THRESHOLD):
        continue  # Skip this window if no dip occurs
...
# Save results to CSV
csv_filename = "biased_investment_strategy_results.csv"
